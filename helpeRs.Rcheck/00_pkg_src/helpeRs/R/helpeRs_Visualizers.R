...
image2 = function(mat,xaxt=NULL,yaxt = NULL,col=NULL,main=NULL,scale_vec=c(1,1.04),cex.axis = 1){
  ...
  graphics::image((t(mat)[,nrow(mat):1]),
                  axes = F, main = main,xaxs = "i",cex.main = 2)
  if(!is.null(xaxt)){ graphics::axis(1, at = 0:(ncol(mat)-1)/ncol(mat)*scale_vec[1], tick=F,labels = (xaxt),cex.axis = cex.axis,las = 1)  }
...
heatMap <- function(x, y, z,
  ...
  if(!grepl(useLog,pattern="z")){
    if(add.legend == TRUE){
      ...
      fields::image.plot(s_, xlab = xlab, ylab = ylab, log = useLog, cex.lab = cex.lab, main = main, cex.main = cex.main,
      ...
      graphics::image(s_, xlab = xlab, ylab = ylab, log = useLog, cex.lab = cex.lab, main = main, cex.main = cex.main,
                      col = myCol, xlim = xlim, ylim = ylim)
    }
  }
  if(grepl(useLog,pattern="z")){
    useLog <- gsub(useLog, pattern="z", replacement="")
    ...
    if(add.legend == TRUE){
      # Use image.plot with full parameter set
      fields::image.plot(s_$x,s_$y,log(s_$z),
      ...
      # Use base image with compatible parameters only
      graphics::image(s_$x,s_$y,log(s_$z), main = main, cex.main = cex.main,
                      xlab = xlab, ylab = ylab, log = useLog, cex.lab = cex.lab,
                      xlim = xlim, ylim = ylim, col = myCol)
    }
  }
  if(!is.null(vline)){ graphics::abline(v=vline,lwd=10,col=col_vline) }
  if(!is.null(hline)){ graphics::abline(h=hline,lwd=10,col=col_hline) }

  if(includeMarginals == T){
    graphics::points(x+stats::rnorm(length(y),sd=marginalJitterSD_x*stats::sd(x)), rep(ylim[1]*1.1,length(y)),pch="|",col="darkgray")
    graphics::points(rep(xlim[1]*1.1,length(x)), y+stats::rnorm(length(y),sd=stats::sd(y)*marginalJitterSD_y),pch="-",col="darkgray")
  }
}
...
heatmap2 <- function(mat, row_labels = NULL, col_labels = NULL,
  ...
  if(!is.null(col_labels)){
    graphics::axis(1, at = seq_len(ncol(mat)), labels = col_labels, las = 2)
  }
  if(!is.null(row_labels)){
    graphics::axis(2, at = seq_len(nrow(mat)), labels = row_labels, las = 1)
  }
  invisible(NULL)
}
...
MakeHeatMap <- function(factor1, factor2, outcome, dat, lm_obj, pdf_path, 
  ...
  dat_new$Yhat <- OutcomeTransformFxn( c(stats::predict(lm_obj, newdata = dat_new)) )
  dat_new$Yhat <- dat_new$Yhat * OUTCOME_SCALER

  grDevices::pdf( pdf_path )
  {
    graphics::par(mar=c(5,5,3,1), mfrow = c(1,1))
